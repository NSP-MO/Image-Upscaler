...
class DATUpscaler(BaseUpscaler):
    """
    PyTorch Neural Upscaler running DAT (Dual Aggregation Transformer) architecture with CUDA acceleration.
    """
    ...
    def _init_model(self):
        ...
        if self.model_path and os.path.exists(self.model_path):
            try:
                ...
                print(f"[DAT] Successfully loaded 100% of Dual Aggregation Transformer weights ({len(state_dict)} tensors) from {self.model_path}")
            except Exception as e:
                print(f"[DAT] Warning loading state dict: {e}")

        self.net.eval()

    def unload_model(self):
        """
        Unloads PyTorch DAT Transformer network from GPU VRAM and clears CUDA memory cache.
        """
        ...
        gc.collect()
        print("[DAT] VRAM and GPU memory successfully freed.")
